...
This is the exact research code used to produce the paper's results — shared for transparency and reproducibility, not maintained as a library. See [Limitations](#limitations).
...
| `EvaluationHandler/` | ROC-AUC / PR-AUC / accuracy / precision / recall / F1 |
...
| `100_datasets/` | Input data goes here (not included — see [Data](#data)) |
| `200_experiments/` | Experiment outputs (created automatically) |

## Installation

Python 3.8+, PyTorch, and:

```bash
pip install torch numpy pandas scikit-learn pyod hdbscan networkx matplotlib seaborn plotly tqdm wandb
```

No pinned versions ship with the original code. `wandb` is only needed if you run with `-wandb True` (default); pass `-wandb False` to skip it. GPU is used automatically when available.

## Data

| `-dataset` | Expected file | Source |
|---|---|---|
| `ey` | `100_datasets/01_ernstyoung/Analytics_mindset_case_studies_PSU_Hotel_data_v1.csv` | EY "Analytics Mindset" teaching case — the only dataset a reader can source independently |
| `sap` | `100_datasets/03_sap/complete_file_4.csv` | Confidential SAP ERP extract from an audit engagement |

**No data is included in this repository** — the SAP (and an unwired `serpro`) extract are confidential engagement data under NDA. Populate `100_datasets/` yourself; if your export uses different columns, adjust the mapping in the corresponding dataset block of `main.py` and add matching preprocessing methods in `DataHandler/DataHandler.py`.
...
- No dependency lock file (see [Installation](#installation)).
- `serpro` and the plain `autoencoder` experiment paths exist but aren't fully wired up / are stubbed out.
- Model checkpoint saving is disabled by default.
- Variable-sized entry graphs are batched via `DataLoader(batch_size=1)` + manual accumulation — simple, not the most efficient.

Contributions that improve reproducibility (a `requirements.txt`, wiring up `serpro`, batching improvements) are welcome.
