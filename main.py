...
def main():
    ...
    parser = argparse.ArgumentParser(description='deepGNN Experiments')

    # general experiment parameter
    parser.add_argument('-exp_timestamp', help='', nargs='?', type=str,  default=dt.datetime.utcnow().strftime('%Y-%m-%d_%M'))
    parser.add_argument('-exp_series', help='', nargs='?', type=int, default=201)
    parser.add_argument('-experiment', help='', nargs='?', type=str,  default='graph_autoencoder') #graph_autoencoder, autoencoder
    ...
    parser.add_argument('-max_line_items', help='', nargs='?', type=int, default=1000)
    parser.add_argument('-no_global_anomalies', help='', nargs='?', type=int, default=100)
    parser.add_argument('-no_local_anomalies', help='', nargs='?', type=int, default=100)
    ...
    parser.add_argument('-seed', type=int, default=4444, help='seed value for deterministic results.')
    parser.add_argument('-encoder_dim', nargs='+', default=[128,64,32,16,8,2], help='the dimensions of the encoder gnn layers.')  # 512,256,128,64,32,16,8,2
    parser.add_argument('-decoder_dim', nargs='+', default=[2,8,16,32,64,128], help='the dimensions of the decoder fc layers.')  # [2, 4, 8, 16, 32, 64, 128] 2, 8,16,32,64,128,256,512
    ...
    parser.add_argument('-train_iterations', type=int, default=500,help='the number of training iterations.')
    parser.add_argument('-train_batch_size', type=int, default=256, help='the training batch size.')
    ...
    parser.add_argument('-valid_batch_size', type=int, default=256, help='the evaluation batch size.')
    ...
    parser.add_argument('-algo', type=str, default='iforest', help='the anomaly detection algorithm.') # lof, ocsvm, iforest, hdbscan, knn, hbos
    ...
    #parser.add_argument('-min_cluster_size', type=int, default=15, help='the mini cluster size for hdbscan.')
    ...
    # case: graph autoencoder experiment
        ...
        if experiment_parameter['dataset'] == 'ey':
            ...
            data_parameter['je_segment_attributes_categorical'] = ['AccountType', 'AccountClass', 'GLAccountNumber',
                                                                   'GLAccountName']
            data_parameter['je_segment_attributes_numerical'] = ['Amount']
            data_parameter['je_attributes'] = data_parameter['je_header_attributes'] + data_parameter[
                'je_segment_attributes_categorical'] + data_parameter['je_segment_attributes_numerical']

            # set journal entry feature information
            data_parameter['je_header_features'] = ['Y_SOURCE', 'Y_PREPARER_ID']
            data_parameter['je_segment_features_categorical'] = ['Y_JE_ITEM_IDENTIFIER', 'Y_DEBIT_CREDIT',
                                                                 'Y_GL_ACCOUNT_TYPE', 'Y_GL_ACCOUNT_CLASS',
                                                                 'Y_GL_ACCOUNT_NUMBER',
                                                                 'Y_GL_ACCOUNT_NAME']  # Y_JE_ITEM_IDENTIFIER and Y_DEBIT_CREDIT are derived features.
            data_parameter['je_segment_features_numerical'] = ['Y_DMBTR']
            data_parameter['je_features'] = data_parameter['je_header_features'] + data_parameter[
                'je_segment_features_categorical'] + data_parameter['je_segment_features_numerical']
            data_parameter['je_segment_features'] = data_parameter['je_segment_features_categorical'] + data_parameter[
                'je_segment_features_numerical']

            # set the visualization features
            data_parameter['hover_attributes'] = ['Y_JE_IDENTIFIER', 'Y_JE_ITEM_IDENTIFIER', 'Y_GL_ACCOUNT_TYPE',
                                                  'Y_GL_ACCOUNT_CLASS', 'Y_GL_ACCOUNT_NAME', 'Y_PREPARER_ID',
                                                  'Y_SOURCE', 'Y_DMBTR', 'Y_CLASS_NAME']
            data_parameter['visual_attributes'] = ['Y_BUZEI', 'Y_PREPARER_ID', 'Y_SOURCE', 'Y_ACCOUNT_TYPE',
                                                   'Y_ACCOUNT_CLASS', 'Y_GL_ACCOUNT_NAME', 'Y_DMBTR']
        ...
        # case: sap dataset
            ...
            # set journal entry attribute information
            data_parameter['je_header_attributes'] = ['DocumentNr', 'DocType', 'UserName Post', 'Transaction'] #delete 'DocTypeDescr'
            data_parameter['je_segment_attributes_categorical'] = ['PostingKey', 'GL Accountnr', 'GL AccountDescr',
                                                                   'ProfitCenter' ,'Year','Month']  #delete 'CostCenter'
            ...
            data_parameter['je_segment_features_categorical'] = ['Y_JE_ITEM_IDENTIFIER', 'Y_DEBIT_CREDIT', 'Y_BSCHL',
                                                                 'Y_HKONT', 'Y_PRCTR', 'Y_YEAR','Y_MONTH','Y_KOSTL', 'Y_HKONT_TEXT'] # delete Y_KOSTL, , 'Y_HKONT_TEXT'
            data_parameter['je_segment_features_numerical'] = ['Y_DMBTR']
            data_parameter['je_features'] = data_parameter['je_header_features'] + data_parameter[
                'je_segment_features_categorical'] + data_parameter['je_segment_features_numerical']
            data_parameter['je_segment_features'] = data_parameter['je_segment_features_categorical'] + data_parameter[
            ...
            # set the visualization features
            data_parameter['hover_attributes'] = [ 'Y_JE_ITEM_IDENTIFIER', 'Y_BLART', 'Y_TCODE',
                                                  'Y_BSCHL',  'Y_DMBTR', 'Y_CLASS_NAME','Y_KOSTL', 'Y_HKONT_TEXT']   # delete Y_KOSTL, 'Y_HKONT_TEXT',
            data_parameter['visual_attributes'] = ['Y_JE_ITEM_IDENTIFIER', 'Y_BLART', 'Y_TCODE', 'Y_BSCHL',
                                                    'Y_DMBTR','Y_KOSTL', 'Y_HKONT_TEXT']  # delete Y_KOSTL, 'Y_HKONT_TEXT',
